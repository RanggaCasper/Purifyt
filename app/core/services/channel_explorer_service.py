...
async def explore_channel_stream(
    ...
    4. If video has judi → immediately yield video_ready with sampled comments
       (ALL judi + judi×1.5 normal for that video)
    ...
    for idx, video in enumerate(videos, 1):
        ...
        # Sample: ALL judi + judi×1.5 normal
    ...
    overall_pct = round((total_judi_saved / total_saved) * 100, 2) if total_saved > 0 else 0

    yield {
        "type": "done",
        "comments": [],  # already saved per-video
